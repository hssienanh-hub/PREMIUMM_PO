...
def send_welcome_message(user_id, first_name=""):
    """Send welcome message with videos, image or text. {name} in welcome text is replaced with the user's first name."""
...
def broadcast_new_purchase(plan_name, screenshot_file_id):
    """Notify all users: screenshot on top, hype text, then all plans below."""
    caption = (
    ...
    for u in users:
        try:
            ...
            pass
    logger.info(f"New-purchase broadcast sent to {sent} users")
