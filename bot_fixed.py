...
BOT_TOKEN = os.getenv("8602849042:AAHCGsAPtTV-MfXr_ZDXkqotLqtbI8h3mEY", "").strip()
ADMIN_IDS = [''8892796335'c]
...
class Database:
    ...
    def delete_welcome_video(self, video_id):
        ...
        rows = c.execute(
            'SELECT id FROM welcome_videos ORDER BY order_num ASC'
        ).fetchall()
        for order_num, (row_id,) in enumerate(rows, start=1):
            c.execute(
                'UPDATE welcome_videos SET order_num = ? WHERE id = ?',
                (order_num, row_id)
            )
        conn.commit()
        conn.close()
...
def _auto_styled_ikb_init(self, text, *args, style=None, **kwargs):
    if style is None:
        t = text or ""
        if any(k in t for k in ["❌", "🗑️", "Reject", "Delete", "Cancel", "🔙", "Back"]):
            style = "danger"
        elif any(k in t for k in ["✅", "Approve", "Done", "PAY", "Pay", "🔊", "CLICK AND JOIN", "|"]):
            style = "success"
        else:
            style = "primary"
    try:
...
def send_welcome_message(user_id, first_name=""):
    """Send welcome media/text and available plans."""
    ...
    # Send configured welcome videos first.
    if welcome_videos:
        try:
            if len(welcome_videos) > 1:
                media_group = [
                    types.InputMediaVideo(v["file_id"])
                    for v in welcome_videos[:10]
                ]
    ...
    # If there are no videos, send the configured image.
    elif WELCOME_IMAGE:
        ...
        # Image already contains the welcome text.
        personalized_text = None

    # Send text when it wasn't already included in the image caption.
    if personalized_text:
        text = f"<b>{BOT_NAME}</b>\n\n{personalized_text}"
        safe_send(user_id, text, reply_markup=main_keyboard(user_id))

    # Always show plans after welcome media/text.
...
def setme_cmd(msg):
    bot.reply_to(msg, "❌ This command is disabled. Configure ADMIN_IDS in the environment.")
        ...
        # ========== APPROVE PAYMENT ==========
            ...
            try:
                ...
                if payment:
                    ...
                    # Notify ALL users about this new purchase (screenshot + hype text + all plans)
                    # Runs in background so the admin's approve click responds instantly.
                    threading.Thread(
                        target=broadcast_new_purchase,
                        args=(plan['name'], payment.get('screenshot_file_id')),
                        daemon=True
                    ).start()
